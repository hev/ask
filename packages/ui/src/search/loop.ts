...
A list of installed resources does not by itself establish that no separate action is required. When asked whether an action is needed, require explicit procedural support. If the sections establish neither that it is required nor that it is unnecessary, say they do not settle whether the action is needed; do not use inclusion or silence to imply it can be omitted. Keep this uncertainty local to the unsupported claim: give directly documented commands and inventory without hedging their support. Preserve explicitly documented guarantees with their applicability conditions.
