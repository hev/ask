...
description: >-
  ...
  graph, KG, or search index, or after docs content changes. Runs `ask kg corpus`,
...
`.hev-ask/digest.json`. Only the **distillation** of the digest needs a model — the
...
4. **Assemble.**
   ...
   deterministically, and writes `.hev-ask/digest.json`.
...
6. **Clean up and commit.** Delete the intermediates and commit the graph:
   ...
   Only `.hev-ask/digest.json` is committed; the input/distill files are scratch.
