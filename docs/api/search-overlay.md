...
>
> If the server has no provider [API key](../api/configuration.md#choosing-a-provider), asking
...
statement that the supplied sections do not establish the requirement or its
absence. Explicit guarantees remain usable with their documented scope, such as
...
custom properties. Define these on `:root` (the hev house values shown) and the
...
function. It serves the same [`POST /api/ask` contract](../api/endpoint.md)
the Astro route does, holds the provider [API key](../api/configuration.md#choosing-a-provider) server-side, and reads
...
wrangler deploy            # set your provider's API key as a secret
```
...
The CLI and [MCP](../api/mcp.md) surfaces are host-neutral on every row: a
coding agent reads the same digest no matter what renders the docs. For the
design behind this split, see
[Concepts](../concepts.md#host-neutral-one-digest-any-framework) and
[Tradeoffs](../tradeoffs.md#a-flagship-adapter-primitives-for-the-rest).
