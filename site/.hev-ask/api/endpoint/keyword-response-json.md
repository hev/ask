...
terms: ["keyword","response","json","mode","returns","envelope","ranked","results","title","optional","heading","group","snippet","alongside","echoed","query","configured","loop","model","warning","present","agentic","requested","result","carries","deep","link","section","anchor","appended","absent","only","document","intro","chunk","concepts","search","docs","overview","reader"]
...
Keyword mode returns a JSON envelope of ranked results (each with title, optional heading, URL, optional group, and a snippet) alongside the echoed query, the configured loop model, the mode that ran, and an optional warning present when agentic was requested but no key is configured. The result URL carries the deep link with the section anchor appended, absent only for a document's intro chunk.
