...
terms: ["claude","code","skill","bundled","build","digest","builds","tree","inside","subscription","instead","provider","running","deterministic","producer","seam","sharded","emit","shard","inputs","distil","fresh","context","synthesize","across","notes","assemble","because","corpus","size","never","hits","limit","result","same","shape","incremental","hash","gate","unattended"]
...
The bundled build-digest skill builds the tree inside your Claude Code subscription instead of using a provider API key, running the deterministic producer seam sharded: emit per-shard inputs, distil each shard in its own fresh context, synthesize across shard notes, then assemble. Because each shard is its own context, corpus size never hits a limit, and the result has the same shape and incremental hash gate as the unattended build.
