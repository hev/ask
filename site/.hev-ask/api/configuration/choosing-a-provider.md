...
terms: ["choosing","provider","option","selects","serves","both","runtime","loop","offline","build","identical","search","behavior","digest","format","endpoint","contract","across","providers","only","model","environment","variable","change","openrouter","reaches","every","routes","through","base","override","opens","openai","compatible","builder","models","must","support","tool","calling"]
...
The provider option selects who serves both the runtime loop and the offline build, with identical search behavior, digest format, and endpoint contract across providers; only the model and key environment variable change. OpenRouter reaches every model it routes through one key, a base-URL override opens any OpenAI-compatible endpoint, and loop and builder models must support tool calling (the builder also needs forced tool choice); a changed endpoint route must be mirrored on the overlay's matching prop.
