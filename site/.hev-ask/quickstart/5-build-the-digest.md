...
terms: ["build","digest","offline","built","markdown","tree","commit","gives","loop","domain","context","ranks","keyword","results","supplies","glossary","holds","suggested","questions","recommended","bundled","claude","code","skill","inside","subscription","token","spend","model","call","outside","provider","flag","picks","serves","verify","both","paths","incremental","hash"]
...
The digest is an offline-built markdown tree you commit that gives the loop domain context, ranks keyword results, supplies the glossary, and holds the suggested questions. Build it the recommended way with the bundled Claude Code skill inside your subscription (no key or token spend) or with the CLI's one model call for CI or outside Claude Code (the provider flag picks who serves it), then verify and commit; both paths are incremental and hash-gated and the integration also runs the build during the site build when a key is present.
