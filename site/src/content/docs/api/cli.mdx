---
title: "CLI"
description: "The ask CLI reads the committed digest as a directory (tree to map it, cat to read, facts for literals, grep to search over path keys) and builds, verifies, and serves it for agents and CI."
group: "API"
order: 3
---
...
[API key](/docs/api/configuration#choosing-a-provider). It runs the deterministic producer seam, sharded:
...
provider [API key](/docs/api/configuration#choosing-a-provider) is present, then falls back to the committed tree if the
