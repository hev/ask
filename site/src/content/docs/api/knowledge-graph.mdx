...
offline by [`ask kg build`](/docs/api/cli) and committed to your repo. (`kg` is
the digest's historical name; it survives in the CLI group, the `--kg-path`
flag, and the virtual module id.) The integration inlines it into the build through the
`virtual:hev-ask/kg` module, so the running site reads it with no filesystem
access.
...
| `nodes` | `KnowledgeNode[]` | One distilled section per doc heading — the body of the digest. |
| `edges` | `KnowledgeEdge[]` | Reserved; ships empty. |
