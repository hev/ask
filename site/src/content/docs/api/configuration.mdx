...
| `kgModel` | `string` | `'claude-opus-4-8'` | Model for the offline digest builder. |
...
| `kgPath` | `string` | `'.hev-ask/digest.json'` | Path to the committed digest artifact, relative to the site root. |
...
- registers two virtual modules — `virtual:hev-ask/config` (the resolved
  options) and `virtual:hev-ask/kg` (the committed digest, inlined);
- adds `kgPath` as a watched file so dev reloads when the digest changes;
