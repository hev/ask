...
import { askTreeDiagram } from "../../lib/diagrams";
...
<Diagram label="the ask digest as a directory">{askTreeDiagram}</Diagram>
...
This site serves both from its own digest.

<Callout label="drop ask into your own CLI">
The digest reads (`tree`, `cat`, `grep`, `facts`) also ship as a
[Go library](/docs/api/cli#go-library) in `pkg/ask`, beyond the bundled `ask`
